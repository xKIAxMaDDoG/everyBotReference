...
ADIS16448_IMU::ADIS16448_IMU(Axis yaw_axis, AHRSAlgorithm algorithm)
  ...
  // SensorBase removed from class in 2019
  //SensorBase::SetName("ADIS16448_IMU", 0);
}
...
void ADIS16448_IMU::Calibrate() {
  ...
  {
    std::lock_guard<wpi::priority_mutex> sync(m_mutex);
    m_accum_count = 0;
    m_accum_gyro_x = 0.0;
    m_accum_gyro_y = 0.0;
    m_accum_gyro_z = 0.0;
...
ADIS16448_IMU::~ADIS16448_IMU() {
  m_freed = true;
// BEGINNING 2019  m_samples_not_empty.notify_all();
...
void ADIS16448_IMU::Acquire() {
  ...
  while (!m_freed) {
    ...
    {
      std::lock_guard<wpi::priority_mutex> sync(m_samples_mutex);
...
void ADIS16448_IMU::Calculate()
{
  while (!m_freed)
  {
    ...
    Sample* sample;
    {
      std::unique_lock<wpi::priority_mutex> sync(m_samples_mutex);
      m_calculate_started = true;

      while (!( m_samples_count != 0 || m_freed))
      {
// BEGINNING 2019      	  m_samples_not_empty.wait(sync);
      }
...
{
#if 0
//	auto table = nt::NetworkTable::
	auto table = nt::NetworkTableInstance::GetTable();
//		  NetworkTableInstance::  GetTable();
  if (table) {
