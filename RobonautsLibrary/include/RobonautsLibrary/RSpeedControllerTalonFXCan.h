...
{
    public:
        RSpeedControllerTalonFXCan(int port_device, int controlPeriodMs);
        ~RSpeedControllerTalonFXCan();
        ...
        void SetStatorCurrentLimit(uint32_t amps, uint32_t peak=0, uint32_t duration=0 );
        void SetStatorCurrentLimitEnabled(bool enabled);
        ...
        bool isForwardLimitPressed(void);
        bool isReverseLimitPressed(void);

        void publish(std::string tab, std::string controller_name);
        ctre::phoenix::motorcontrol::can::BaseMotorController * GetMotorControllerCtre() ;
        rev::CANSparkMax * GetMotorControllerSparkMax() { return 0; };

        ControllerType GetSpeedControllerType() { return(TALON_FX); };

    private:
        ctre::phoenix::motorcontrol::can::WPI_TalonFX m_controller;
};
