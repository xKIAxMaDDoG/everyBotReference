 ...
 *
 * No intintionl changes have been made to the functionallity of the 
				...
				while (p < _end)
				{
					...
					else if ((_flags & NEEDS_ENTITY_PROCESSING) && *p == '&')
					{
						...
						if (*(p + 1) == '#')
						{
							...
							{ 0 };
							int len;
							p = const_cast<char*> (XMLUtil::GetCharacterRef(p,
							    buf, &len));
							for (int i = 0; i < len; ++i)
							{
								*q++ = buf[i];
...
#pragma warning (pop)
	...
	const XMLElement* XMLNode::NextSiblingElement(const char* value) const
	{
		...
		{
			if (element->ToElement() && (!value || XMLUtil::StringEqual(value,
			    element->Value())))
			{
	...
	XMLAttribute* XMLElement::FindOrCreateAttribute(const char* name)
	{
		...
		for (attrib = _rootAttribute; attrib; last = attrib, attrib
		    = attrib->_next)
		{
	...
	XMLPrinter::XMLPrinter(FILE* file, bool compact) :
		_elementJustOpened(false), _firstElement(true), _fp(file), _depth(0),
