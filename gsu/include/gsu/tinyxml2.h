 ...
 *  distribution.
 *  
 * Incorprated into this project by: 
 * 	The Robonauts
 * 	FRC Team 118
 * 	NASA, Johnson Space Center
 * 	Clear Creek Independent School District
 *
 * No intentional changes have been made to the functionallity of the 
...
namespace tinyxml2
{
	...
	class XMLNode
	{
		...
		public:
			...
			const XMLElement
			    * PreviousSiblingElement(const char* value = 0) const ;
	...
	class XMLHandle
	{
		public:
			...
			{
				return XMLHandle(
				    _node ? _node->PreviousSiblingElement(_value) : 0);
			}
			...
			{
				return ((_node && _node->ToDeclaration()) ? _node->ToDeclaration()
				    : 0);
			}
		...
		public:
			...
			const XMLConstHandle FirstChildElement(const char* value = 0) const
			{
				return XMLConstHandle(
				    _node ? _node->FirstChildElement(value) : 0);
			}
			...
			{
				return XMLConstHandle(
				    _node ? _node->LastChildElement(_value) : 0);
			}
			...
			{
				return XMLConstHandle(
				    _node ? _node->NextSiblingElement(_value) : 0);
			}
